...
scan_host() {
    ...
    for OCTET in $OCTETS; do
        ...
        echo "[*] nmap -Pn --min-rate=300 ${PORT_FLAG} -v --unprivileged -oN ${OUTDIR}/allporttcp_${TARGET} $TARGET"
        nmap -Pn --min-rate=300 ${PORT_FLAG} -v --unprivileged -oN "${OUTDIR}/allporttcp_${TARGET}" $TARGET
        PORTS=$(cat "${OUTDIR}/allporttcp_${TARGET}" | grep -E ^[0-9] | cut -d'/' -f1 | tr '\n' ',' | sed 's/,$//')
        echo "[i] ------------- done -------------"
        echo "[*] nmap -Pn -sVC -p${PORTS} -v --unprivileged -oN ${OUTDIR}/allporttcpver_${TARGET} $TARGET"
        nmap -Pn -sVC -p${PORTS} -v --unprivileged -oN "${OUTDIR}/allporttcpver_${TARGET}" $TARGET
        echo "[i] ------------- done -------------"
    done
}
