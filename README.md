...
## Disclaimer
All testing has been done on ubuntu 24.04.

## Scripts
### update-hosts.sh:
Wrote this quick script to take in a CIDR range and add the host name and the IP address to your /etc/hosts file.
...
The above command will xor a csharp style shellcode with the key "z" and output nothing except the formatted xor'd array which I passed to clip (alias around xclip)
...
## Loaders
### Hollow
Shellcode loader using the Process Hollowing technique. This starts an instance of notepad.exe and injects the shellcode into that.

### Ntmapsec
Shellcode loader using the NtMapViewOfSection technique. This starts injects the shellcode into an already existing process which the user can choose.

### Linux loader
A shellcode loader for linux written in C which fetches the shellcode from a remote server and injects it into a forked process so even if the parent process dies, the shellcode keeps running.

### ASPX
A slightly modified version of metasploit aspx shellcode loader which now fetches the shellcode from a remote server and injects it into the current process.